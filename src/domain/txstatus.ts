 ...
 *
 * Source: symbol-openapi `spec/core/transaction/schemas/TransactionStatusEnum.yml` (main branch,
 * fetched 2026-09-13): the `enum` list in file order, and the one-line explanation each code has
 * in that file's `description` block. Codes the file lists without an explanation (`Success`,
 * `Neutral`, `Failure`, `Failure_Hash_Already_Exists`) map to null; nothing here is paraphrased
 * or added from memory.
 *
 * The transaction groups come from `TransactionGroupEnum.yml` in the same directory.
 */

export const TRANSACTION_GROUPS = ['confirmed', 'unconfirmed', 'partial', 'failed'] as const;
...
const TABLE: ReadonlyArray<readonly [string, string | null]> = [
  ...
  [
    ...
    'Validation failed because an entity originated from the nemesis account after the nemesis (first) block.',
  ],
  [
  ...
  [
    ...
    'Validation failed because the block does not match the schema of an importance block.',
  ],
  ...
  ['Failure_Hash_Already_Exists', null],
  [
  ...
  [
    'Failure_Aggregate_V1_Prohibited',
    'Validation failed because aggregate transaction with version 1 is not allowed.',
  ],
  [
    'Failure_Aggregate_V2_Prohibited',
    'Validation failed because aggregate transaction with version 2 is not allowed.',
  ],
  [
    'Failure_Aggregate_V3_Prohibited',
    'Validation failed because aggregate transaction with version 3 is not allowed.',
  ],
  [
  ...
  [
    ...
    'Validation failed because an eternal namespace was received after the nemesis (first) block.',
  ],
  [
  ...
  [
    ...
    'Validation failed because the operation type is incompatible. *Note*: This indicates that the existing restrictions have a different operation type than that specified in the notification.',
  ],
  [
  ...
  [
    ...
    'Validation failed because the maximum number of restrictions would be exceeded.',
  ],
  [
  ...
  [
    ...
    'Validation failed because the remote chain has an improper importance link.',
  ],
  [
...
/** Every code of the OpenAPI enum, in file order, with its explanation (null when the file has none). */
export const TRANSACTION_STATUS_CODES: ReadonlyMap<string, string | null> = new Map(TABLE);

/** Explanation of a status code, or null for `Success`, codes without one, and unknown codes. */
export function describeTransactionStatusCode(code: string): string | null {
  return TRANSACTION_STATUS_CODES.get(code) ?? null;
}
