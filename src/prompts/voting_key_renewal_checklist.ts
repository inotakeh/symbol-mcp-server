...
export const votingKeyRenewalChecklistPrompt = definePrompt({
  ...
  description:
    "Step-by-step renewal of a Symbol voting key: check the current key and free slots, confirm the node is synced and not lagging, verify the operator's VotingKeyLink transaction by hash, confirm the new key is registered, and later confirm it actually votes. The server never signs or announces; the operator runs the link commands.",
...
6. Once the new key's startEpoch has been finalized (compare it with the current finalization epoch), call symbol_finality_participation with account "{account}" and confirm the epoch is reported as "participated" (both prevote and precommit signed). If it is "missed", the key is registered but the node is not voting with it: tell the operator to check the voting node's key file and sync state.
7. Finish with exactly four lines: Current key / New key / Expiry / Open items.`,
});
