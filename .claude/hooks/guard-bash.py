...
PROTECTED_PATHS = re.compile(r"(\.claude/|\.github/workflows/|CLAUDE\.md|AGENTS\.md|\.npmrc|package-lock\.json|LICENSE|SECURITY\.md|CODEOWNERS|server\.json|mcpb/manifest\.json|\.gitignore)")
...
def main():
    ...
    if PROTECTED_PATHS.search(flat) and WRITE_OPS.search(flat):
        print("BLOCKED by guard-bash: shell writes to protected files (.claude/, .github/workflows/, CLAUDE.md, AGENTS.md, .npmrc, package-lock.json, LICENSE, SECURITY.md, CODEOWNERS, server.json, mcpb/manifest.json, .gitignore) are forbidden. Propose the change in chat for a human to apply.", file=sys.stderr)
        sys.exit(2)
