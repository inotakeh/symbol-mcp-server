...
- verdict（`deriveVersionDriftVerdict`）: 自ノードの版が未知（own が null）→ `unknown`／size 0 → `unknown`（hint: ピア接続自体の問題。`symbol_node_health` / `symbol_node_status` を見る、または `SYMBOL_REFERENCE_NODES`。設定済みなら notes を見る）／`newerShare ≥ 0.75`（`FAR_BEHIND_SHARE`）→ `far_behind`（「接続を拒否され始める可能性」）／`own < majority` **または** `newerShare ≥ 0.5`（`BEHIND_SHARE`。自ノードが最頻値でも新しい版の合計が過半なら取り残されつつある）→ `behind`／それ以外 `ok`（多数派より新しい場合も ok）。閾値はポリシー定数で、ネットワーク定数ではない。
