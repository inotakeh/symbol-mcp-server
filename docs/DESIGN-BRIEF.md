...
4. **出力は構造化＋人間向け**。全ツールに `outputSchema` を定義し、`structuredContent` と、その JSON 文字列を入れた `text` ブロックの**両方**を返す（仕様の後方互換要件。例外は `symbol_harvesting_income` の `output: csv` で、text が CSV 本文になる。§5.2）。`structuredContent` の先頭に `summary: string`（1 行目で問いに答える短い要約。月ごとの行や ok でないチェック、除去数の行などを後ろに足してよい）を必ず含める。金額は divisibility 適用後の値と生の整数の両方。日時は ISO 8601（UTC）＋ `SYMBOL_TIMEZONE` 指定時はローカル時刻も。数値IDは名前解決（例: `6BED913FA20223F8` → `symbol.xym`、`16724` → `transfer`）。
...
| MCP SDK | **`@modelcontextprotocol/server`** v2 系（完全一致で固定。版は package.json） | 2026-07-27 公開の安定版。v1 の `@modelcontextprotocol/sdk` は保守のみ |
| スキーマ | **`zod`** v4 系（同じく完全一致で固定）、`import * as z from 'zod/v4'` | SDK v2 の依存。`inputSchema` には `z.object(...)` を渡す（v1 のように shape を渡さない） |
...
| テスト | vitest 5（開発時は Node 22.12 以上）。SDK公式のインプロセス方式（§7） | TypeScript SDK リポジトリの `docs/testing.md`（§12） |
...
- `voting_key_renewal_checklist`: `symbol_voting_key_status`（endEpoch・失効予定・推奨ウィンドウ・空き枠）→ `symbol_node_status`（未同期なら中止）→ `symbol_network_compare` → 更新コマンドは人間が実行 → 教えられた Tx ハッシュを `symbol_transaction_status` で confirmed 確認 → `symbol_voting_key_status` を再確認（新キーが active / future、失効キーが unlink されて枠が空いた）→ 新キーの startEpoch が確定した後で `symbol_finality_participation`（そのエポックが `participated`＝prevote と precommit の両方に署名。`missed` なら鍵は登録済みだがノードがその鍵で投票していないので、投票ノードの鍵ファイルと同期を確かめるよう伝える）→ 「現行キー / 新キー / 失効予定 / 未対応事項」の 4 行。
- `monthly_health_check`: `symbol_node_status` → `symbol_node_health`（verdict と warn 以上の項目。unhealthy なら先頭）→ `symbol_version_drift`（verdict・自ノード版・majorityVersion。behind 以上なら先頭）→ `symbol_network_compare` → `symbol_harvester_watch`（mode `compare_and_save`。`current.count` と `comparison` の previousCount / previousTakenAt / 追加・削除件数 / deltaCount。負なら注意。`comparison` が null で notes が `SYMBOL_STATE_DIR` 未設定を言うなら前回値をユーザーに聞き、初回なら baseline 保存を報告。`symbol_harvesting_status` は制限値や鍵一覧を求められたときだけ）→ `symbol_voting_key_status`（30 日以内なら警告を先頭）→ `symbol_account_get`（残高 vs `minVoterBalance`）→ `symbol_harvesting_income`（先月 1 日〜末日、daily。`totals.xym`・`totals.receipts` と日平均のあと、`totals.blocksHarvested`（自分でハーベストしたブロック）と `totals.blocksBeneficiaryOnly`（委任者などのブロック）を分けて報告する。beneficiary のレシート数は委任者のブロック数ではないと明記）→ 要対応 / 注意 / 正常の 3 段階で 1 画面。

## 4. 設定

| 環境変数 | 必須 | 内容 |
|---|---|---|
| `SYMBOL_NODE_URL` | **必須** | 例 `https://<node-host>:3001`。末尾スラッシュの有無を吸収。`https://` 必須（`http://` は `localhost` / `127.0.0.1` / `[::1]` のみ許可）。認証情報（`user:pass@`）・クエリ・フラグメントを含む URL は `ConfigError`（`src/config.ts` の `validateNodeUrl`） |
| `SYMBOL_NETWORK` | 任意 | `mainnet` / `testnet`。指定時は起動時に照合、不一致なら起動失敗 |
| `SYMBOL_TIMEZONE` | 任意 | IANA名（例 `Asia/Tokyo`）。日時出力にローカル時刻を併記 |
| `SYMBOL_REFERENCE_NODES` | 任意 | カンマ区切りURL。各 URL は `SYMBOL_NODE_URL` と同じ規則（`validateNodeUrl`）で検証する。`symbol_network_compare` の比較対象と、`symbol_version_drift` の版の標本（参照ノードの `/node/info` だけ。CLI check の version_drift も同じ）。**ここに無いURLへは通信しない** |
| `SYMBOL_REQUEST_TIMEOUT_MS` | 任意 | 既定 10000。100〜600000 の整数（範囲外や整数でない値は `ConfigError`） |
...
- **account 引数の解決規則**（0.2.0 で追加。`src/tools/_accounts.ts` の `resolveAccountInput`。account を受ける全ツール＝`symbol_account_get` / `symbol_voting_key_status` / `symbol_harvesting_status` / `symbol_harvesting_income` / `symbol_finality_participation` / `symbol_delegation_diagnose` / `symbol_account_rank` / `symbol_holdings_value` / `symbol_transaction_search` の `address` / `symbol_address_parse` の `value` が通る）:
  1. 判定順（`classifyAccountId`）: 64 桁 hex 公開鍵 → 48 桁 hex アドレス → 39 文字 base32 アドレス（**チェックサムが通ったときだけ**）→ ネームスペース名（`[a-z0-9][a-z0-9_-]*` を `.` で最大 3 階層、各部分 64 文字以下。SDK `idGenerator.js` と `NamespaceNetworkPropertiesDTO` の `maxNameSize` / `maxNamespaceDepth` で確認。`classifyAccountId` は入力の前後の空白を削ってからどの形式も判定し（`' alice.pay '` は `alice.pay`）、名前はそれ以外は書かれたとおりに判定して小文字化しない（`ALICE` や `alice pay` は不正））→ どれでもなければ `isError`＋ヒント（ヒントに「ネームスペース名も可」）。小文字 39 文字でチェックサムが通らない入力はネームスペース判定へフォールスルーする（39 文字ちょうどの正当な名前を「不正なアドレス」と誤判定しないため）。大文字の打ち間違いは両方の規則に落ちて invalid のまま。小文字の 64 桁 hex は名前としても合法だが公開鍵が先に勝つ。
...
**`symbol_account_get`** — `account`（base32 アドレス・48 桁 hex アドレス・64 桁 hex 公開鍵・アドレスエイリアスのあるネームスペース名。§5 共通規約の解決規則）、`format`。
...
**`symbol_transaction_search`** — `address`（アドレス・公開鍵・ネームスペース名。§5 共通規約の解決規則。署名者か受取人としてかかわる取引）、`type`（任意、名前または数値）、`pageSize`（10〜100、既定 10）、`pageNumber`（1〜10,000、既定 1）、`order`（desc既定）、`format`。
上記と同じ形式の要約リスト＋ページング情報（`pagination.nextPageNumber`。続きがありうるときは summary に次の `pageNumber` を書く）。
参照: `GET /transactions/confirmed?address=...&type=...&pageSize=...&pageNumber=...&order=desc`。
...
**`symbol_address_parse`** — `value`（base32アドレス / hexアドレス / 公開鍵 / ネームスペース名。名前のときだけノードに問い合わせる。§5 共通規約）。
...
**`symbol_voting_key_status`** ← **最重要ツール** — `account`（アドレス・公開鍵・ネームスペース名。§5 共通規約の解決規則）。
- 登録済み Voting キー一覧: publicKey、startEpoch、endEpoch、状態（`expired` / `active` / `future`）
- 現在のファイナライズエポック、現在高さ
- アクティブキーと未開始（future）のキーについて: 残りエポック数、残りブロック数、**残り日数**、**失効予定日時**（ISO/UTC＋ローカル）、**推奨実施ウィンドウ**（失効の7日前〜3日前）。future のキーには開始予定日時（`startsAt`）も付ける（失効済みのキーにはどれも付けない）
...
- 次のとき `warnings[]` に 1 件ずつ足す: アクティブキーが無い（future キーがあれば次のキーの開始エポックを添える）／アクティブキーが 30 日以内に失効し、切れ目なく引き継ぐ future キーが無い（`src/domain/voting.ts` の `hasSuccessorKey`。§13 の check と同じ規則）／残高が `minVoterBalance` 未満／登録枠の空きが 0
計算方法は §6。

**`symbol_harvesting_status`** — `account`（任意。アドレス・公開鍵・ネームスペース名。§5 共通規約の解決規則）。
...
**`symbol_harvesting_income`**（0.2.0 で追加。ツール配列の末尾に登録し、既存の順序を変えない）— `account`（アドレス・公開鍵・ネームスペース名。§5 共通規約の解決規則）、期間は `fromDate`/`toDate`（`YYYY-MM-DD`、両端含む。`SYMBOL_TIMEZONE` の日付、未指定なら UTC）または `fromHeight`/`toHeight` の**どちらか一方**（両方・どちらも無し・片方だけ・逆順・実在しない日付は `isError`）、`granularity`（`daily` 既定 / `monthly` / `receipt`）、`format`（JSON の詳細度。`receipt` 時の上限: concise 50 件 / detailed 500 件）、`output`（`json` 既定 / `csv`。text ブロックの形式。`format` とは独立）。
- 指定期間にそのアカウント宛に発生した HarvestFee（8515。OpenAPI の `ReceiptTypeEnum` での表記は Harvest_Fee）レシートのうち、通貨モザイク（`/network/properties` の `currencyMosaicId`）のものだけを集計する。件数・合計は**サーバー内で BigInt により決定的に計算**し、divisibility 適用後の文字列と生の整数を両方返す。summary の数値もその文字列を埋め込む（LLM に計算させない、丸めない）。
...
- 出力: `summary`、`period`（種別・日付・使用タイムゾーン）、`range`（fromHeight/toHeight と両端ブロックの日時）、`totals` と `daily[]`（`SYMBOL_TIMEZONE` の日付境界でバケット）または `receipts[]`、各行に harvester / beneficiary / unknown の内訳（`xym*` と `raw*`）とブロックの数、`truncated`、`notes`（「ハーベストは確率的で日ごとの変動が大きい」「為替換算は含まない」、取り分とブロックの定義）。
- **ブロックの数**（0.9.0 で追加）: totals・daily[]・monthly[] の各行に `blocks`（このアカウントがレシートを受け取ったブロック）、`blocksHarvested`（自分でハーベストしたブロック＝harvester の取り分を受け取った。常に `receiptsHarvester` と同じ）、`blocksBeneficiaryOnly`（他のアカウントがハーベストし、beneficiary の取り分だけを受け取ったブロック。多くはこのアカウントのノードの委任者）。分類できなかったステートメントのブロックは `blocks` にだけ入る。レシートは取り分であってブロックではない。自分のノードの beneficiary でもある運用者は、自分のブロック 1 つにつき harvester と beneficiary の 2 件を受け取るので、`receiptsBeneficiary` は委任者のブロック数ではない（Claude Desktop が「beneficiary 357 件＝委任者のブロック」と誤読した実例。実際は 357 件のうち 317 件が自分のブロックの取り分で、委任者のブロックは 40）。そこで summary の 2 行目に「Blocks: N harvested by this account, M harvested by others that paid it only the beneficiary share …」と、自分のブロック由来の beneficiary レシートの数を書く。説明文と notes の beneficiary の定義も「ハーベストしたノードが指定する beneficiary への取り分。自分のノードの beneficiary なら自分のブロックからも受け取る」とした（以前の「他人のブロック」は誤り）。集計は高さごとに 1 ブロックとして数え（`src/domain/harvesting.ts` の `addBlock`）、1 ブロックのレシートは同じ日付に入るので「月の件数 = その月の日次の件数の合計」も成り立つ。`addBlock` は各ブロックを harvested / beneficiary only / unrecognised のちょうど 1 つに数え、自分のブロックの beneficiary レシート（`beneficiaryReceiptsInOwnBlocks`）と分類できなかったブロック（`blocksUnrecognised`）もその場で数える（どちらも出力フィールドではなく summary 用。ツール側で差し引きから推定しない）。ブロックは空でない配列の型（`[HarvestRow, ...HarvestRow[]]`）で扱う。
...
**`symbol_delegation_diagnose`**（0.2.0 で追加。ツール配列の末尾に登録し、既存の順序を変えない）— `account`（アドレス・公開鍵・ネームスペース名。メインアカウントを渡す）、`recentDays`（1〜30、既定 7。直近のハーベスト実績を探す日数）、`format`（concise は ok のチェックの `hint` を null、detailed は全チェックに `hint`）。
...
- 閾値はすべて `/network/properties` から取る: `minHarvesterBalance` / `maxHarvesterBalance`（**`chain.harvestingMosaicId` の残高で比較**。OpenAPI `ChainPropertiesDTO` に「Mosaic id used to provide harvesting ability」として存在。mainnet / testnet では `currencyMosaicId` と同じ値だがハーベスト要件は harvestingMosaicId で定義されている。パーサは必須扱いで（`parseHexId`）、16 桁の hex でなければ `/network/properties` の読み取りが失敗するので、フォールバックは置かない）、`importanceGrouping`、`harvestBeneficiaryPercentage` / `harvestNetworkPercentage`（レシート分類）。
- チェック（`checks[]` 固定順、各 `{ id, status: ok|warn|fail|unknown, detail, hint }`）:
  ...
  10. `recent_harvest` — 直近 `recentDays` 日の HarvestFee（8515）レシート。期間→高さは実測平均ブロック時間（`getAverageBlockTime`）からの推定（`notes` に明記。二分探索を使わないのはリクエスト数のため）。`/statements/transaction?...&order=desc` を 100 件ページで最大 20 ページ。集めたステートメントを `symbol_harvesting_income` と同じ `aggregateHarvestIncome` に通し、**harvester の分のレシートだけを数える**（ハーベストしたブロック 1 つにつき 1 件。自分のノードの beneficiary でもある自分のブロックも 1。beneficiary の分は他人がハーベストした証拠なので除外）。分類できなかったブロック（`blocksUnrecognised`）は数えず、detail の文で別に書く（0.9.1 で修正。以前は unknown も数えたので、分類できない自分のブロックを 2、委任者のブロックを「ハーベストした」と数えていた）。判定: 1 件以上 ok（分類できなかったブロックがあれば detail の末尾に「Not counted: …」）。0 件で分類できなかったブロックが 1 以上なら unknown（hint: 比率が harvestBeneficiaryPercentage / harvestNetworkPercentage と合わず、ハーベストしたのか beneficiary だけかを判定できない。ほかに fail が無ければ verdict は cannot_verify）。0 件でそれも無く fail が無ければ warn（「有効だが直近 N 日は当たっていない。importance が小さいと間隔が空く」）、fail があれば unknown。20 ページで打ち切ったら notes に「読んだのは新しい 2,000 ステートメント（harvester でも beneficiary でも、報酬を受け取ったブロック）で、数は下限」。`recentHarvest { days, receipts, lastHeight, lastTime }`（`receipts` は harvester のレシート数＝ハーベストしたブロック数。フィールドの形は不変）。
...
このツールの出力に `hint` フィールドは無い。この節と §7 の「助言」は、verdict が ok 以外のときに summary の 2 行目に置く `- ` で始まる 1 行のこと（detailed で後ろに続く分布の行とは別。§13 の CLI check はこの行を hint にする）。
...
- 版 0（0.0.0.0）は「版が未知」で、実在の版ではない（0.9.0 で追加）。catapult は peers ファイル（`knownPeers`）から読んだピアを `NodeMetadata(networkFingerprint, name)` で作り、版は `NodeVersion()` つまり 0 のまま（`client/catapult/src/catapult/config/PeersConfiguration.cpp`、`ionet/Node.h`）。`/node/peers` はノードが本当の版を知るまで 0 を返す。そうしたピアと参照ノード（`isUnreportedVersion`）は分布・多数派・`newerShare` に入れず、`sample.unknownVersion` で数える（1 以上なら summary の 1 行目の末尾と notes に書く）。以前は 1 つの版として数えていたので、標本を水増しして `newerShare` を下げ、0 が多いと多数派になって自ノードが「多数派より新しい」＝ ok と判定されえた。ピアの版がすべて 0 なら標本が空なので `unknown` で、助言は「ピアがまだ版を報告していない」。notes は出所で分ける: ピアは peers ファイルの説明、参照ノードは「自分の `/node/info` で 0 を返した」。標本が空で `SYMBOL_REFERENCE_NODES` が設定済みなら、助言は設定を勧めず「どの参照ノードも使える版を返さなかった（notes 参照）」と書く。自ノードの `/node/info` が 0 を返したときも同じく未知として扱う（`versionDistribution` と `deriveVersionDriftVerdict` に own を null で渡す）: verdict は `unknown`、`newerShare` は null、分布と `majorityVersion` はそのまま返し、助言は「ノード側で版を確かめて majorityVersion と比べる」。0 と比べると標本の全ノードが新しい版になり、far_behind（「すぐに更新」）の誤報になるため。
...
- verdict（`deriveVersionDriftVerdict`）: 自ノードの版が未知（own が null）→ `unknown`／size 0 → `unknown`（助言: ピア接続自体の問題。`symbol_node_health` / `symbol_node_status` を見る、または `SYMBOL_REFERENCE_NODES`。設定済みなら notes を見る）／`newerShare ≥ 0.75`（`FAR_BEHIND_SHARE`）→ `far_behind`（「接続を拒否され始める可能性」）／`own < majority` **または** `newerShare ≥ 0.5`（`BEHIND_SHARE`。自ノードが最頻値でも新しい版の合計が過半なら取り残されつつある）→ `behind`／それ以外 `ok`（多数派より新しい場合も ok）。閾値はポリシー定数で、ネットワーク定数ではない。
- 出力（先頭 summary）: `summary` / `network` / `verdict` / `node { version, versionRaw, restVersion: nullable }` / `sample { size, source: 'peers' | 'peers+reference', peers, referenceNodes, ignored, unknownVersion }`（`size`・`peers`・`referenceNodes` は版が分かって分布に入った数） / `distribution [{ version, count, share }]` / `majorityVersion: nullable` / `newerShare: nullable` / `farBehindShare` / `notes[]`（サンプルは自ノードが知っているピアの一部で全体像は nodewatch／ピアの host・friendlyName・publicKey は untrusted で**出力しない**（version と数だけ）／除外規則）。
...
- 処理: `ctx.getNetworkData()` → モザイク解決（既定は currency の id と alias を使い、divisibility と供給量は `GET /mosaics/{id}` から毎回取得。キャッシュしない）→ `account` があれば `fetchAccount`（`GET /accounts/{id}`。残高はここから、順位は走査から。404 は既存のヒント付き `isError`）→ `GET /accounts?mosaicId=&orderBy=balance&order=desc&pageSize=100&pageNumber=n` を 1 ページ目から**逐次**（同時リクエスト 1 本）。停止条件: address（hex）が一致 → `rank = (n-1)×100 + index + 1`、`rankBeyond: null`／`ceil(maxRank/100)` ページまで無ければ `rank: null`、`rankBeyond: maxRank`／100 件未満のページで一覧が尽きた、または残高が 0 なら `rank: null`、`rankBeyond: null`（残高 0 は 1 ページ目で止める）。account が無ければ 1 ページ目だけ。各ページの `RestError` はそのまま失敗（隠さない、部分結果を返さない）。
...
**`symbol_holdings_value`**（0.7.0 で追加。ツール配列の末尾に登録し、既存の順序を変えない）— `account`（必須。アドレス・公開鍵・ネームスペース名）、`unitPrice`（必須。**文字列**。10 進表記の正の数、小数点以下最大 12 桁、全体で最大 40 文字（`MAX_PRICE_LENGTH`。正規化の前の長さ）。41 文字以上・指数表記・カンマ・通貨記号・符号・`12.`・`.5`・空・0 は `run` 内で検証して `isError`＋ヒント。zod の `.min(1)` や regex に任せると SDK の汎用メッセージになりヒントが付かないので schema は `z.string()` のまま）、`currency`（必須。`^[A-Z]{3,6}$`。同じく `run` 内で検証）、`priceSource`（任意。最大 80 文字。untrusted として `text.clean`（§2-8 の除去と計数の経路。除去した文字は `invisibleCharactersRemoved` に数える）を通して echo。何も残らなければ指定なし（null））、`priceAsOf`（任意。最大 40 文字。`text.clean` で除去した後に `Date.parse` が通ることだけ検証し、除去後の値を書式を変えずに echo（何も残らなければ指定なし）。ISO 8601 の厳密な正規表現は入れない）、`mosaic`（任意。hex id かエイリアス名。既定は通貨モザイク。`resolveMosaicInput` を再利用し、通貨モザイクなら追加リクエストなし）、`decimals`（任意。0〜12 の整数。`amount` を丸める桁。schema は `z.number()` のままで `run` 内で検証して `isError`＋ヒント）、`format`（JSON は同一。detailed は、丸めたときだけ summary に丸め前の値の 1 行を追加）。
...
- **サイズ上限はストリームで適用する**。Content-Length が上限を超えれば本文を読まずに拒否し、読み取り中も累計バイトを数えて超えた時点で reader を cancel する（圧縮応答は展開後のバイトで数える）。読まない本文（HTTP エラー・リダイレクト・Content-Length 超過）は `body.cancel()` で破棄し、接続を持ち続けない。
- **ルートが無い 404 はエラーにする**（0.9.1 で追加。Fail loudly）。404 の本文は同じ上限の下で読み、判定にだけ使う（文言には入れない）。restify の「ルートが無い」形（`code` が `ResourceNotFound` で `message` が `/` で始まり ` does not exist` で終わる）なら `RestError('route_not_found')` にし、`getOrNull` も null にしない（ツールは isError で「ノードがこのエンドポイントを提供していない。このサーバーの不具合か、対応していない catapult-rest の版」）。`no resource exists with id '…'`・空の本文・それ以外の形は従来どおり `not_found`（`getOrNull` は null）。0.9.0 までの `symbol_account_get` は存在しない `/accounts/{address}/multisig` を叩き、この 404 を「マルチシグではない」と読んでいた。テストのハーネスも、スタブの無いパスには同じ「ルートが無い」404 を返すので、パスを取り違えると null や not_found を期待するテストでも落ちる。「データが無い」を期待するテストは、そのパスを `resourceNotFound(id)` で明示的にスタブする（ワイルドカードは使わない。`/accounts/*` のような前方一致は、まさにこの取り違えを隠す）。
...
テストの置き場所: `test/unit/`（純粋関数・CLI・スクリプト・文書の検査）、`test/tools/`（インプロセスの MCP クライアントで呼ぶツール層。ツールごとのファイルと全ツール横断の検査、共通のハーネス `harness.ts`）、`test/evals/`（`evals/cases.json` の検査）、`test/integration/`（実ノード。下の「統合」）。フィクスチャは `test/fixtures/`（規則は `test/fixtures/README.md`）。以下は要件と落とし穴を種類ごとに挙げたもので、テストファイルの目録ではない（モジュールごと・ツールごとの基本のテストは個別に書いていない）。
...
- ハーベスト報酬のブロックの数（`src/domain/harvesting.ts`）: 自分が beneficiary でもあるブロック（2 レシートで 1 ブロック、blocksHarvested）、他人のブロック（beneficiary の 1 レシート、blocksBeneficiaryOnly）、beneficiary なしの (100-N):N（blocksHarvested）、分類できないステートメント（blocksUnrecognised）、自分のブロックの beneficiary レシート（beneficiaryReceiptsInOwnBlocks）、月のブロック数と両カウンタ = その月の日次の合計。ツール層では、取り込んだ実データ（11 ブロック: 9 と 2）で totals・daily・summary の 2 行目・月の行・CSV の末尾 3 列を固定値で確かめ、合成の分類不能ステートメントで 2 行目と月の行の「not recognised」を確かめる
...
- バージョン比較（`src/domain/version.ts`）: `isUnreportedVersion`（0 だけが未知、0.0.0.1 は報告済み）、own が null（自ノードの版が未知）なら分布と多数派はそのままで `newerShare` は null、verdict は標本によらず unknown、`compareVersions`（`1.0.3.10 > 1.0.3.9`、欠け成分は 0）、分布の順序と同数タイブレーク、`newerShare`、verdict（own が最頻値 40% でも newerShare 0.6 → behind、0.7499 → behind、0.75 → far_behind、空 → unknown）
...
- `RestClient` の HTTP 衛生（`test/unit/rest.test.ts`）: fetch に `redirect: 'manual'` を渡す / 3xx（301・302・303・307・308）と opaqueredirect は `redirect` エラーで、本文を読まずに破棄し、Location を文言に入れない / ローカルの実ソケットで 302 を返しても転送先に接続しない / ほかの 3xx（300・304）は HTTP エラーのまま / HTTP エラー・Content-Length 超過（404 も）の本文は読まずに破棄 / ルートが無い 404（`"/… does not exist"`）は `route_not_found` で `getOrNull` も投げ、`no resource exists with id` の 404 は `not_found`（`getOrNull` は null）、判定は restify の形ちょうどだけ（否定例 9 種）、上限を超えて流れ続ける 404 の本文は打ち切って `not_found` / 上限を超え続けるストリームは途中で打ち切る（引き出した量を検査。cancel が失敗しても `too_large`）/ 安全でないパス（`..`・`?#`・空白・`%`・`\`・`//`）は送らずに拒否
- 委任診断の規則（`src/domain/delegation.ts`）: verdict（fail 優先 / unknown → cannot_verify / warn のみ → active）、importance 再計算までの残りブロック（ちょうど倍数のときは G）、鍵有無、委任要求マーカー判定（マーカーのみ / 先頭 0xFE だが不一致 / 平文）、残高の範囲 `classifyHarvesterBalance`（min ちょうどと max ちょうどは within、1 単位外れると below / above。harvesting_status と diagnose が共用）
...
- provenance の待機（`test/unit/wait-for-attestations.test.ts`）: `scripts/wait-for-attestations.sh` を wait-for-npm と同じく sleep を差し替えて、npm レジストリの代わりのローカル HTTP サーバーに対して実行する。すぐ取れれば保存／404 と 5xx の間は聞き直す／`NPM_WAIT`（既定 300 秒、15 秒ごと）で諦める／403・401・410 は待たずに失敗／応答が無いときも聞き直し、最後のメッセージにそう書く／引数の誤りと秒の整数でない `NPM_WAIT` はリクエストせずに exit 2、を確かめる
- リリースノートの抽出（`test/unit/release-notes.test.ts`）: `scripts/release-notes.mjs <version>` を子プロセスで実際の `CHANGELOG.md` に対して実行し、日付付きの節の本文だけを出す（見出しと次の節は含めない、最初の見出しの前の注記は残す、最後の節はリンク参照の前で止まる）こと、無い版と Unreleased は exit 1、引数が 1 つでなければ exit 2 を確かめる
...
- `symbol_version_drift`: 既定フィクスチャ（6 ピア: 1.0.3.9 ×4 / 1.0.3.8 / 1.0.4.0）で ok と分布 / 自ノード 1.0.3.8 と 7 ピア（1.0.4.0 ×3、1.0.3.8 ×2、1.0.3.7 ×2）→ behind（newerShare 0.43 でも多数派より古い）/ 全ピア 1.0.4.0 → far_behind / 同数タイブレーク → 新しい版が多数派 / ピア 0 件と `/node/peers` 503 → unknown（助言に SYMBOL_REFERENCE_NODES）/ 参照ノード A が新版・B が失敗 → `peers+reference`、host 集合 = node + A + B、参照側のパスは `/node/info` のみ / 参照ノードが testnet seed → 除外 / 自ノード鍵・別 seed・壊れた要素 → `ignored` 3 / `/node/server` 503 → `restVersion` null / 出力テキストにピアの host・friendlyName・publicKey が無い / 版 0 のピア 3 件を足しても分布・多数派・newerShare は不変で `unknownVersion` 3、summary の 1 行目の末尾と notes / 自ノード 1.0.3.8・ピア 1.0.4.0 ×2 と 0 ×4 → far_behind（0 を版として数えると ok になっていた）/ 全ピアが 0 → unknown と専用の助言 / 参照ノードの版 0 → `unknownVersion` 1 と参照ノード用の note（peers ファイルの説明は出ない）/ 参照ノードを設定したが版 0 だけ → 助言は notes を指し、設定を勧めない / 自ノードの版 0 → unknown・`newerShare` null・`majorityVersion` はそのまま、detailed は分布の行を出す / outputSchema
...
- スキーマの移植性（`test/tools/schema_portability.test.ts`）: `tools/list` の全ツールの inputSchema と outputSchema に `type` の配列（zod の `.nullable()` が出す `{"type":["string","null"]}`）が無く、nullable は `anyOf` と null の枝で表す。`additionalProperties`・`items`・union の枝に何も制約しないスキーマ（`{}` / `true`）が無い（一部のクライアントが `type` の配列を拒否し、Inspector は制約の無いスキーマを警告するため）
...
**evals（`test/evals/cases.test.ts`。CIで必ず実行。LLM もネットワークも使わない）**
- `evals/cases.json` が 10 件以上で id が重複せず、対象者（node-operator / general）の両方を含み、登録された全ツールを 1 回以上参照し、各ケースの `expectedArguments` がそのツールの inputSchema を通る（スキーマに無いキーは不可）

**統合（`SYMBOL_INTEGRATION=1` のときだけ。CI既定では走らせない）**
`test/integration/live.test.ts`。`SYMBOL_NODE_URL`（testnet でも mainnet でもよい）のノードに実際に接続し、ツールはツール層と同じインプロセスの MCP クライアントで呼ぶ。`SYMBOL_INTEGRATION=1` では単体とツール層のテストも一緒に走る（`vitest.config.ts`）。
- 呼ぶツールがどれもエラーなく応答し、structuredContent の先頭が `summary` で outputSchema を満たす。呼ぶのは `symbol_network_info`・`symbol_node_status`・`symbol_account_get`・`symbol_voting_key_status`・`symbol_transaction_search`・`symbol_transaction_get`・`symbol_namespace_get`・`symbol_mosaic_get`・`symbol_fee_estimate`・`symbol_address_parse`・`symbol_time_convert`・`symbol_harvesting_status`・`symbol_network_compare` の 13 で、`SYMBOL_INTEGRATION_ACCOUNT` の指定時は `symbol_finality_participation`・`symbol_harvesting_income` も加わる。`symbol_transaction_status`・`symbol_delegation_diagnose`・`symbol_node_health`・`symbol_version_drift`・`symbol_harvester_watch`・`symbol_account_rank`・`symbol_holdings_value` の 7 つは統合テストの対象外（ツール層テストだけで確かめる）
- アカウントを受けるツールは（下のマルチシグの検査を除き）`SYMBOL_INTEGRATION_ACCOUNT` で呼び、未設定ならノード自身のメインアカウント（`symbol_node_status` の `node.publicKey`）で呼ぶ。この変数が要るテスト（下の Voting キーの件数・ファイナリティ参加・ハーベスト報酬）は、未設定なら skip
- 中身の検査: `symbol_transaction_search` の最初の行のハッシュを `symbol_transaction_get` で引くと confirmed で同じ種別名、ゼロのハッシュは `not_found` / `type: 'transfer'` の検索は全行が 16724 / `symbol_network_info` の通貨の別名を `symbol_namespace_get` と `symbol_mosaic_get`（名前と id）で引くと同じ mosaicId と divisibility（別名が無いネットワークでは何もしない）/ `symbol_address_parse` はそのアカウントを有効で設定したネットワークのものと判定 / `symbol_time_convert` は確定高さのエポックが `/chain/info` と同じで推定ではなく、そのエポックの高さ範囲が確定高さを含み、未来の高さは推定 / `symbol_harvesting_status` は account なしとありの両方 / `symbol_network_compare` の先頭行が自ノードで到達でき、`referenceNodesConfigured` が設定と一致
...
- 同じ `SYMBOL_INTEGRATION_ACCOUNT` に対し、`symbol_finality_participation` が最新確定エポックで `participated` か `missed` のいずれかを返す（`unavailable` でない。未設定なら skip）
- 同じ `SYMBOL_INTEGRATION_ACCOUNT` に対し、`symbol_harvesting_income` が直近 3 日（`fromDate` = 2 日前、`toDate` = 今日。UTC で数えた日付）で `totals.receipts` ≥ 1、`daily` 1〜4 行を返す（ハーベストしているアカウントを指定する。未設定なら skip）
...
│   ├── cli/                # check サブコマンドの判定（runCheck・対応表・decideExit）と text / JSON の整形。MCP SDK を import しない（§13）
│   ├── context.ts          # AppContext と createAppContext（サーバーと check で共通の起動手順）
│   ├── server.ts           # createServer(): McpServer（TOOLS と PROMPTS の登録）
│   ├── instructions.ts     # SERVER_INSTRUCTIONS（initialize 結果に載せる本文、§3.1）
│   ├── prompts/            # 1ファイル1プロンプト（§3.1）
│   ├── config.ts           # env 読込・URL検証・ネットワーク照合
│   ├── client/             # REST クライアント（fetch ラッパ: timeout, UA, サイズ上限, スキーマ検証。§6 HTTP衛生）とノード応答の zod スキーマ
│   ├── domain/             # I/O の無い純粋関数のドメインロジック（アドレス・金額・エポック・文字列の除去・集計・判定など）。単体テストは test/unit/
│   ├── state/              # snapshotfile.ts: src で唯一 node:fs を使う（symbol_harvester_watch の状態ファイル、§2-9）
│   └── tools/              # 1ファイル1ツール（symbol_*.ts）、各ファイルで input/output zod を定義。`_` で始まるファイルはツール共通の補助（defineTool、account やモザイクの解決など）
├── test/                   # unit/, tools/, evals/, integration/, fixtures/（§7）
├── evals/                  # 実際の質問文と期待するツール呼び出しの例（Phase 3。test/evals/ が検査する）
├── scripts/                # リリース（版の検査・リリースノート・npm の待機・配布物の取得）と .mcpb のビルド、フィクスチャの取り込み、アイコンの生成（§7・§14）
├── mcpb/                   # .mcpb の manifest.json（保護対象のテンプレート）と icon.png（§14）
├── docs/                   # この文書、RELEASING.md（リリースごとの手順）、保守者の環境の補助ファイル
├── server.json             # MCP Registry 用
├── package.json  LICENSE  README.md  README.ja.md  CHANGELOG.md  SECURITY.md  CONTRIBUTING.md
├── AGENTS.md  CLAUDE.md  GUARDRAILS.md  .claude/   # AI エージェント向けの指示とガードレール（フックと設定）
└── .github/                # workflows/（CI・リリース・OpenSSF Scorecard）、dependabot.yml、CODEOWNERS、Issue と PR のテンプレート
```
...
- dependencies: `@modelcontextprotocol/server`、`zod`。devDependencies: typescript、vitest、biome（`@biomejs/biome`）、`@modelcontextprotocol/client`（テスト用）、`@types/node`。どれも `^` を付けず完全一致で固定する（版は package.json と lockfile が正）

**README.md（英語）**: 何ができるか、インストール（`npx` 一行）、Claude Desktop / Claude Code の設定例（JSON。`env` に `SYMBOL_NODE_URL`）、環境変数、ツール一覧と実行例、**セキュリティ声明（読み取り専用・秘密鍵を扱わない・指定ノード以外に通信しない）**、対応ネットワーク、制限事項。日本語版 `README.ja.md` も置き、README.md と同じく全ツール名と全環境変数を含め、`## ` の節の数をそろえる（`test/unit/docs-sync.test.ts` が検査する。§7）。
...
| MCP Registry | `server.json`（`release.yml` の `registry` ジョブが GitHub OIDC で `mcp-publisher publish`。失敗したときは失敗したジョブを再実行するか（npm がまだ新しい版を見せていなかったときなど）、人間が手作業で公開する（`docs/RELEASING.md` の「When something fails」）。§8） | Registry からサーバーを探すクライアント |
...
**Node.js**: Claude Desktop は Node.js を同梱して node 型の拡張を動かす（公式）。同梱の版は公式ドキュメントに書かれていない。`compatibility.runtimes.node` は `engines` と同じ ">=22" にし、`claude_desktop` の下限は公式に確認できる値が無いので書かない。確認済み事項（2026-09-23）: 人間が実機の Claude Desktop に .mcpb を入れ、動作を確認した。そのときの Claude Desktop の版と、ログに出る Node の版は記録が無い。
