...
- **account 引数の解決規則**（0.2.0 で追加。`src/tools/_accounts.ts` の `resolveAccountInput`。account を受ける全ツール＝`symbol_account_get` / `symbol_voting_key_status` / `symbol_harvesting_status` / `symbol_harvesting_income` / `symbol_finality_participation` / `symbol_delegation_diagnose` / `symbol_transaction_search` の `address` / `symbol_address_parse` の `value` が通る）:
