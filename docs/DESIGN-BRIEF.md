...
- ハーベスト報酬のブロックの数（`src/domain/harvesting.ts`）: 自分が beneficiary でもあるブロック（2 レシートで 1 ブロック、blocksHarvested）、他人のブロック（beneficiary の 1 レシート、blocksBeneficiaryOnly）、beneficiary なしの (100-N):N（blocksHarvested）、分類できないステートメント（blocks だけ）、月のブロック数 = その月の日次の合計。ツール層では、取り込んだ実データ（11 ブロック: 9 と 2）で totals・daily・summary の 2 行目・月の行・CSV の末尾 3 列を固定値で確かめ、合成の分類不能ステートメントで「not recognised」を確かめる
