...
- バージョン比較（`src/domain/version.ts`）: `isUnreportedVersion`（0 だけが未知、0.0.0.1 は報告済み）、`compareVersions`（`1.0.3.10 > 1.0.3.9`、欠け成分は 0）、分布の順序と同数タイブレーク、`newerShare`、verdict（own が最頻値 40% でも newerShare 0.6 → behind、0.7499 → behind、0.75 → far_behind、空 → unknown）
