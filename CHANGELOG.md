...
- README (English and Japanese): the bundle (.mcpb) does not start without the node URL, and
  ...
  Troubleshooting row for HTTP 429 and 503. `CONTRIBUTING.md`: the same Inspector command, and
  that the Claude Code hooks need Python 3 while contributing by hand needs none of `.claude/`.
