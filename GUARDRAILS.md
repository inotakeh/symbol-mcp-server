...
| T5 | **ガードレールの自己解除**: `.claude/`・フック・ワークフロー・`.npmrc`・lockfile をエージェントが書き換える | Edit deny＋フック＋サンドボックス `denyWrite`（三重。`denyWrite` に無い `.github/CODEOWNERS`・`.github/dependabot.yml`・`package-lock.json` は Edit deny＋フックの二重）／Claude Code 自体も `.claude` を protected path として扱う／CI の protected-files ジョブ／CODEOWNERS |
...
├── .gitignore                       # settings.local.json / CLAUDE.local.md / .env / 鍵 / .claude/private-identifiers.txt
...
│   ├── private-identifiers.txt      # 運用者を特定する文字列（コミットしない。人間が管理。§2.2 の scan-secrets と push 前の検査）
│   └── hooks/
...
│   ├── ISSUE_TEMPLATE/              # Issue フォーム（バグ報告・機能要望。セキュリティ報告は非公開の報告へ）
│   ├── pull_request_template.md     # PR テンプレート
│   └── workflows/
│       ├── ci.yml                   # lint/typecheck/test/build/audit + dependency-review + 保護ファイル検査
│       ├── release.yml              # v* タグ → Environment 承認 → publish → github-release と registry（下記）
│       └── scorecard.yml            # OpenSSF Scorecard（main への push と毎週）
└── docs/
    ├── DESIGN-BRIEF.md              # MCP サーバーの設計ブリーフ（既存文書）
    ├── RELEASING.md                 # リリースの手順（保守者向け）
    ├── pre-push-private-check.sh    # push 前の識別子の検査（人間が .git/hooks/pre-push に入れる。§2.2）
    └── user-settings-snippet.json   # ~/.claude/settings.json に足す分（ユーザー層でしか効かない設定）
```

`release.yml` の流れ: `v*.*.*` のタグ → GitHub Environment `npm-publish` の承認 → `publish`（リリース用ファイルと版の検査、lint・typecheck・テスト・build、`npm publish --provenance`。npm には OIDC で認証）→ `github-release`（CHANGELOG の節をノートに、npm の tarball と provenance、.mcpb と build provenance を添付）と `registry`（MCP Registry に GitHub OIDC で公開）が並行。手順と失敗したときの対処は `docs/RELEASING.md`。
...
- **確認要求（permissionDecision: ask）**: 通常の `git push`、`rebase/merge/cherry-pick`、作業を捨てる git 操作（省略形やまとめた短いフラグを含む: `clean --forc`、`branch -df`）、`gh pr create/edit/close/reopen/comment/ready`、`gh issue create/comment`、`gh run rerun/cancel`、`git fetch --prune-tags`、`npm update` — auto モードでも必ず人間に出る
  - **作業ツリーを別の版に切り替える git の操作**（`checkout <ref>`・`checkout <ref> -- <paths>`（絶対パスのパス指定を含む）、`switch`、`reset`（`--soft` 以外）、`restore`（`--source`（省略形を含む）・`--staged`）、`merge`、`rebase`（`--onto` の先とは直接比べる）、`cherry-pick`（各コミットの変更）、`stash pop/apply/branch`）は、保護ファイル（フック・設定・ワークフローなど）の内容が変わるときに確認にする。B-1 の後は、これらはサンドボックスの中で動き、作業ツリーの保護ファイルのうち `denyWrite` にあるものの書き換えはサンドボックスが拒否するので、それらについての確認は 1 枚目の防御ではなく**案内**になった（`denyWrite` に無い CODEOWNERS・`dependabot.yml`・lockfile については、この確認が今も 1 枚目の防御）。残る意味は、保護ファイルに差がある切り替えが「HEAD と index は移るが、作業ツリーの保護ファイルは元のまま」の中途半端な状態になり、戻すのに `git switch -f` が要ること（B-1 の試行で確認）。判定はフックの中で読み取り専用の `git diff --name-only`（`stash show`、`rev-parse`）を実行して行う。この git は外部プログラムを起動しないように、`-c core.fsmonitor= -c core.hooksPath=/dev/null`、`--no-ext-diff --no-textconv`、`GIT_CONFIG_NOSYSTEM=1` を付け、呼び出し側の `GIT_*` をすべて消して 2 秒の制限時間で実行する。判定できないとき（ref が解決できない、git が失敗した、時間切れ）も確認にする。`git pull` は、取り込む中身が fetch の後にしか分からないので常に確認にする（`fetch` だけなら確認しない）。pull は人間が行う前提
- **通過（exit 0）**: それ以外。permissions ルールと分類器に委ねる

`guard-files.py` は `.claude/**`、`CLAUDE.md`、`CLAUDE.local.md`、`AGENTS.md`、`.mcp.json`、ワークフロー、CODEOWNERS、`.github/dependabot.yml`、`SECURITY.md`、`LICENSE`（`LICENSE.*` を含む）、`server.json`、`mcpb/manifest.json`、`.npmrc`、lockfile（`package-lock.json`・`pnpm-lock.yaml`・`yarn.lock`）、`.git/**` と git フックの設定（`.husky/**`・`lefthook.yml`・`.pre-commit-config.yaml`）、`.env*`（`.env.example` は許可）、鍵ファイル、`harvesters.dat`、**プロジェクト外のパス**（`~/.claude/plans/`・`~/.claude/projects/<project>/memory/`・一時ディレクトリ・セッションの scratchpad を除く）への Edit/Write を止める。正本は `guard-files.py` の `PROTECTED`。
...
`scan-secrets.py` の最初の検査は、運用者を特定する文字列（private identifiers）: `.claude/private-identifiers.txt`（gitignore でコミットしない。人間が管理）があれば、空行・`#` の行・6 文字未満の行を除いた各行を、前後の空白を削った文字どおりの文字列として、大文字小文字を区別せずに書いたファイルから探し、見つかれば exit 2 で止める。`secrets-scan:ignore-file` の印があっても外れない。ノードのホスト名・アドレス・公開鍵・ホームディレクトリの絶対パスのように、公開リポジトリに出してはいけない値を入れる。Edit/Write はプロジェクトの外（scratchpad・メモ）も検査するので、メモや PR 本文の下書きにもホームディレクトリの絶対パスを書かない（`~/` で書く）。push の前にも同じリストで検査する: 人間が `docs/pre-push-private-check.sh` を `.git/hooks/pre-push` に入れると、送るコミットのツリーとコミットメッセージにリストの文字列があれば push を止める。
...
- 記録の更新は人間が行う。Claude Code の外のターミナルでも、`!` でもよい（`!` のコマンドには PreToolUse フックが効かないので、不一致の間でも `record` を実行できる。2026-09-25 に確認）:
  ...
  - `record` は、最後に記録した組から変わったファイルの名前（new / removed / changed）だけを表示して `[y/N]` を聞く（`y` 以外は記録しない。端末でないときは `--yes` が要る）。中身の差は表示しないので、差は git で読む。`.claude/settings.local.json` は gitignore で git に差が残らないので、record のたびに中身の控えをリポジトリの外（例: 記録と同じ `~/.claude/hooks/symbol-mcp-server/`）に取っておくと、次に見張りが止めたときに `diff` で前回との差を読める。

## 3. GitHub リポジトリ側の設定（人間が行う。エージェントには権限を与えない）

作成直後に、リポジトリの Settings で:

1. **Rules → Rulesets → New branch ruleset**（対象 `main`）: Require a pull request before merging（approvals 0 でよい。目的は PR 経由の強制）、Require status checks to pass（`test (22)`, `test (24)`, `dependency-review`, `protected-files`。`test (…)` の名前は `ci.yml` の `test` ジョブのマトリクス `node: [22, 24]` から決まるので、マトリクスを変えたらここも直す）、Block force pushes、Restrict deletions、Require linear history。可能なら Require signed commits。
...
6. **CODEOWNERS と `ci.yml` の `MAINTAINER`**: どちらにもメンテナの GitHub ユーザー名を書く（`protected-files` は、`MAINTAINER` と Dependabot 以外が開いた PR の保護ファイルの変更を拒否する）。メンテナが変わったら両方を直す。
7. **アクションの SHA 固定**: ワークフローのアクションはすべてフルコミット SHA に固定し、バージョンをコメントで添える（例 `actions/checkout@<40 桁の SHA> # vX.Y.Z`。GitHub の公式推奨）。アクションを足すときも同じ形にする。Dependabot（github-actions、月次）が SHA とコメントを一緒に更新する。
...
- リリース: `package.json` の version を上げる PR を merge → あなたが live テスト（`SYMBOL_INTEGRATION=1`。CI では走らない）が緑なのを確かめ、`git tag -l vX.Y.Z` が空であることを確かめてから `git tag vX.Y.Z && git push origin vX.Y.Z` → Environment 承認 → publish（`docs/RELEASING.md`。同じ名前のローカルのタグが先にあれば、それは確認していないコミットを指しうる）。
...
- **`excludedCommands` の一致規則**は公式に書かれていない。2026-09-24 に Claude Code 2.1.281 で、結果が中と外で変わる読むだけのコマンドを使って確かめた: 外で動いたのは、パターンに一致するコマンドを**単独で**書いたとき（`gh --version`、`git ls-remote origin HEAD`）だけ。`;` や `&&` でつないだ行（`gh --version; ls …`）、前にオプションがある形（`git -C . ls-remote origin HEAD`）、コマンド置換を含む行（`gh pr view … --jq "$(…)"`）は、行全体がサンドボックスの中で動いた（gh は設定を読めずに失敗し、SSH の git はプロキシに拒否された）。パイプとリダイレクトも同じ（以前の観察）。2026-09-25 にも、`cd <リポジトリ> && git push …` にパイプを付けた行が中で動いて SSH がプロキシに拒否されたこと、`gh … --jq` の式の中の `|` だけでも（引用符の中でも）gh が中で動いて設定を読めなかったことを観測した。作業ディレクトリは最初からリポジトリなので `cd` は要らない。git push と gh は、ほかに何も付けずに単独で打つ（AGENTS.md にも書く）。したがって今は、外で動くコマンドの行のコマンド置換が外で実行されることはなく、フックの置換の規則（§2.2）は Claude Code の挙動が変わったときのための備え。PR の本文は `--body-file <scratchpad のファイル>` で渡す（`"$(cat <<'EOF' …)"` はフックは通すが、行がサンドボックスで動くので gh が失敗する）。スクリプトやインラインコードから起動した git と gh もサンドボックスの中で動く。
...
- **呼び出しの間の状態**: フックは 1 回の Bash 呼び出しのコマンド行だけを見る。環境変数は呼び出しの間で残らないことを 2026-09-24 に Claude Code 2.1.281 で確認した（1 回目 `export ZZ_T=1`、2 回目 `echo "${ZZ_T:-unset}"` が `unset`）。そのため環境変数の規則は「git や gh と同じ行」に限っている。Claude Code の更新で残るようになったら、`EXEC_ENV_NAME` の変数の `export`・`declare -x`・`typeset -x`・`set -a` を行に関係なく止める必要がある。作業ディレクトリは残るので、git と gh はフック入力の `cwd` で判定している。
...
- **フックが作業ツリーから読まれる問題**: 根本の対策のうち、手元で完結する git をサンドボックスに入れること（B-1）は採用済みで、エージェントの git は `denyWrite` に入れた保護ファイルを書き換えられない（`denyWrite` に無い CODEOWNERS・`dependabot.yml`・lockfile は、フックが止める）。作業ツリーの切り替えの確認は、中途半端な状態を避けるための案内として残している。判定に使う git は 2 秒で打ち切るので、とても大きなリポジトリでは判定できずに確認になることがある。`test-hooks.sh` は、この判定の結果がリポジトリの状態に左右されないように、テストの中で小さな git リポジトリを作ってそこで確かめる。
- **見張りのフックの限界**（§2.4）:
